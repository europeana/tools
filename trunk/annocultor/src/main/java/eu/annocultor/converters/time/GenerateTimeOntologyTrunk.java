 ...
 */
package eu.annocultor.converters.time;
...
import eu.annocultor.context.Concepts.ANNOCULTOR;
...
public class GenerateTimeOntologyTrunk 
{
    ...
    throws Exception
    {		
        bc = new ReportList<LabelTemplate>(new File("."), "milleniums-bc.xml", 10000);
        bc.load();
        ad = new ReportList<LabelTemplate>(new File("."), "milleniums-ad.xml", 10000);
        ad.load();
        centuryTemplates = new ReportList<LabelTemplate>(new File("."), "centuries.xml", 10000);
        centuryTemplates.load();
        halfsTemplates = new ReportList<LabelTemplate>(new File("."), "halfs.xml", 10000);
        halfsTemplates.load();
        thirdsTemplates = new ReportList<LabelTemplate>(new File("."), "thirds.xml", 10000);
        thirdsTemplates.load();
        quartersTemplates = new ReportList<LabelTemplate>(new File("."), "quarters.xml", 10000);
        quartersTemplates.load();
        decadesTemplates = new ReportList<LabelTemplate>(new File("."), "decades.xml", 10000);
        decadesTemplates.load();

        Namespaces namespaces = new Namespaces();
        SesameWriter centuries = SesameWriter.createRDFXMLWriter(
        ...
        centuries.startRDF();
        SesameWriter years = SesameWriter.createRDFXMLWriter(
                new File ("time.years.rdf"), 
                namespaces, 
                "time", 
                "AnnoCultor time ontology, generated trunk: years",
                1024,
                1024
        );
        years.startRDF();
        ...
        for (int century = 1; century < 22; century++) {
            ...
            write(centuries, cent, list("AD" + (first ? "1" : "2") + "xxx"), altLabels);
            ...
            for (int yearThisCentury = 0; yearThisCentury < 100; yearThisCentury++) {
                ...
                int third = yearThisCentury < 33 ? 1 : (yearThisCentury < 66 ? 2 : 3);
                int quart = yearThisCentury < 25 ? 1 : (yearThisCentury < 50 ? 2 : (yearThisCentury < 75 ? 3 : 4));
                int decade = yearThisCentury / 10;

                boolean showYear = year != 0;
                boolean showHalf = yearThisCentury == 0 || yearThisCentury == 50;
                boolean showThird = yearThisCentury == 0 || yearThisCentury == 33 || yearThisCentury == 66;
                boolean showQuart = yearThisCentury == 0 || yearThisCentury == 25 || yearThisCentury == 50 || yearThisCentury == 75;
                ...
                if (showHalf) {
                    ...
                    writeBeginEndDates(centuries, halfId, centuryStartYear + (half - 1) * 50, centuryStartYear + half * 50);
                }
                ...
                if (showThird) {
                    ...
                    writeBeginEndDates(centuries, thirdId, centuryStartYear + (third - 1) * 33, centuryStartYear + third * 33 - 1 + (third / 3));
                }
                ...
                if (showQuart) {
                    ...
                    writeBeginEndDates(centuries, quartId, centuryStartYear + (quart - 1) * 25, centuryStartYear + quart * 25 - 1);
                }
                ...
                if (showDecade) { 
                    ...
                    for (LabelTemplate template : decadesTemplates) {
                        altLabels.add(applyTemplate(decade + 1, century, template));
                    }
                    if (century > 10) {
                        altLabels.add(new LiteralValue(((century - 1) * 10 + decade) + "x"));
                        altLabels.add(new LiteralValue(((century - 1) * 10 + decade) + "?"));
                        altLabels.add(new LiteralValue(((century - 1) * 10 + decade) + "0s"));
                    }
                    write(centuries, decadeId, list(cent, halfId), altLabels);
                    writeBeginEndDates(centuries, decadeId, centuryStartYear + decade * 10, centuryStartYear + (decade + 1) * 10 - 1);
                }

                // years
                if (showYear) {
                    String zeroed = (yearThisCentury < 10 ? "0" : "") + yearThisCentury;
                    altLabels.clear();
                    altLabels.add(new LiteralValue("" + year));
                    String yearId = centStub + zeroed;
                    List<String> parents = list(cent, halfId, thirdId, quartId, decadeId);
    ...
    private static LiteralValue applyTemplate(int part, int whole, LabelTemplate template) {
        ...
        return new LiteralValue(
                (whole > 0 ? String.format(activeTemplate, part, whole) : String.format(activeTemplate, part)), 
                template.getLang().isEmpty() ? null : Lang.valueOf(template.getLang()));

    }

    public static void write(SesameWriter writer, String id, List<String> broader, List<LiteralValue> prefLabel) throws Exception {

        for (String broaderUri : broader) {
            writer.handleTriple(
                    new Triple(NS_TIME + id, Concepts.DCTEMRS.IS_PART_OF, new ResourceValue(NS_TIME + broaderUri), null)
            );                  
        }
        for (LiteralValue literalValue : prefLabel) {
            writer.handleTriple(
                    new Triple(NS_TIME + id, SKOS.LABEL_PREFERRED, literalValue, null)
            );					
        }
        writer.handleTriple(
                new Triple(NS_TIME + id, RDF.TYPE, new ResourceValue(NS_TIME + "Period"), null)
        );
    }

    public static void writeBeginEndDates(SesameWriter writer, String id, int begin, int end) throws Exception {

        writer.handleTriple(
                new Triple(NS_TIME + id, ANNOCULTOR.DATE_BEGIN, new LiteralValue("" + begin + "-01-01"), null)
        );		
        writer.handleTriple(
                new Triple(NS_TIME + id, ANNOCULTOR.DATE_END, new LiteralValue("" + end + "-12-31"), null)
        );		
    }
}
