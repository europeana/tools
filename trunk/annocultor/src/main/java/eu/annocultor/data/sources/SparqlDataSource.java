...
public class SparqlDataSource extends AbstractQueryDataSource
{
	...
	public SparqlDataSource(Environment environment, String sparqlQuery, String... file) throws IOException {
	...
	throws Exception {
		...
		try {
			Helper.importRDFXMLFile(rdf, "http://localhost/namespace", srcFiles.toArray(new File[]{}));
	...
	throws Exception {
		...
		TupleQueryResult resultSet = connection.prepareTupleQuery(getQueryLanguage(), query).evaluate();
		...
		try {
			...
			while (resultSet.hasNext()) {
				...
				Value identifier = bindingSet.getValue(recordIdentifyingPath.getPath());
				if (identifier == null) {
					throw new NullPointerException("On path " + recordIdentifyingPath + " binding set: " + bindingSet); 
				}
				...
				for (String columnName : bindingNames) {
					...
					if (openrdfValue != null) {
						...
						if (openrdfValue instanceof LiteralImpl) {
						    ...
						    if (lang != null) {
						        System.out.println("LANG " + lang + " on " + columnValue);
						    }
						}
						String preprocessedValue = preprocessValue(columnName, columnValue);
						if (preprocessedValue != null) {
						    
							flatHandler.addField(columnName, new LiteralValue(preprocessedValue, lang));
						}
					}
				}
			}
