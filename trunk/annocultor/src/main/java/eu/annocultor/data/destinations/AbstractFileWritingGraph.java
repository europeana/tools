...
import eu.annocultor.triple.ValueHelper;
...
public abstract class AbstractFileWritingGraph extends AbstractGraph
{
     ...
     */
    public AbstractFileWritingGraph(
            String datasetId,
    ...
    {
        return new File(environment.getOutputDir(), getId() + "." + volume + "." + extension);
    }
    ...
    private String makePropertySourcePath(Triple triple)
    {
        ...
        + "\n    source: "
        + (triple.getRule() == null ? "NORULE" : ((triple.getRule().getSourcePath() == null ? "NOPATH" : triple
                .getRule()
                .getSourcePath()
                .getPath())));
    }
    ...
    private void checkTriple(final Triple t) throws Exception
    {
        ...
        if (t.getSubject() == null)
            throw new Exception("TripleBuffer Rejects triple with NULL subject " + t);
        if (t.getSubject().length() == 0)
            throw new Exception("TripleBuffer Rejects triple with empty subject " + t);
        if (!(t.getSubject().startsWith("http://") || t.getSubject().startsWith("https://")))
            throw new Exception("TripleBuffer Rejects triple with subject that does not start with 'http(s)://' " + t);
        if (t.getSubject().contains("#http://") || t.getSubject().contains("/http://"))
            throw new Exception("Found suspicious pattern in subject #http:// or /http://, suggesting a mistake " + t);

        // property
        if (t.getProperty() == null)
            throw new Exception("TripleBuffer Rejects triple with NULL property, " + t.getSubject());
        SesameWriter.checkPredicateUri(t.getProperty().getUri());

        // value
        if (t.getValue() == null)
            throw new Exception("TripleBuffer Rejects triple with NULL value, " + t.getSubject());
        if (t.getValue().getValue() == null)
            throw new Exception("TripleBuffer Rejects triple with NULL value of value, " + t.getSubject());
        if (t.getValue().getValue().contains("#http://") || t.getValue().getValue().contains("/http://"))
            log.debug("Found suspicious pattern in value #http:// or /http://, suggesting a mistake " + t);

        if (ValueHelper.isResource(t.getValue()))
        {
            if (t.getValue().getValue().contains(";"))
                throw new Exception("TripleBuffer Rejects resource triple with ; in resource value, " + t);
            if (t.getValue().getValue().length() == 0)
                throw new Exception("TripleBuffer Rejects resource triple with zero-length of value " + t);
            if (!(t.getValue().getValue().startsWith("http://") || t.getValue().getValue().startsWith("https://")))
                throw new Exception("TripleBuffer Rejects resource triple with value that does not start with 'http(s)://' " + t);
        }
    }
    ...
    public final void add(Triple triple) throws Exception
    {
        ...
        {		
            ...
            catch (Exception e)
            {
                ...
                String msg =
                    "Failed committing the following triple to Sesame "
                    + triple
                    + "\n"
    ...
    public static void createTempDocToShowWhileWorking(File file) throws IOException {
