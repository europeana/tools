...
<xsl:stylesheet 
	...
	xmlns:skos="http://www.w3.org/2004/02/skos/core#"
	 xmlns:repox="http://repox.ist.utl.pt"
	version="1.0">
	...
	<xsl:template match="rdf:RDF/ore:Aggregation">
		<record>
		  <xsl:variable name="id" select="edm:aggregatedCHO/@rdf:resource"/>
		 
		  <europeana:fake>
		  	<xsl:value-of select="@rdf:about"></xsl:value-of>
		  </europeana:fake>
		  <xsl:for-each select="edm:hasView">
				<xsl:variable name="hasView" select="@rdf:resource"/>
				<xsl:for-each select="//rdf:RDF/edm:WebResource[@rdf:about=$hasView]/dc:rights">
				<dc:rights><xsl:value-of select="."></xsl:value-of></dc:rights>
				</xsl:for-each>
			</xsl:for-each>
			<xsl:for-each select="edm:rights">
		  	...
		  	<xsl:for-each select="//rdf:RDF/edm:ProvidedCHO[@rdf:about=$id]">
	...
	<xsl:template name="map_other_properties">
		...
		<xsl:variable name="spatial"  select="@rdf:resource"/>
			<xsl:call-template name="create_elements_from_property">
				<xsl:with-param name="tgt_property">dcterms:spatial</xsl:with-param>
			</xsl:call-template>
			<xsl:for-each select="//rdf:RDF/edm:Place[@rdf:about=$spatial]/skos:prefLabel">
			<dcterms:spatial><xsl:value-of select="."></xsl:value-of></dcterms:spatial>
			</xsl:for-each>
			<xsl:for-each select="//rdf:RDF/edm:Place[@rdf:about=$spatial]/skos:note">
			<dcterms:spatial><xsl:value-of select="."></xsl:value-of></dcterms:spatial>
			</xsl:for-each>
		</xsl:for-each>
	...
	<xsl:template name="create_elements_from_property">
		...
		<xsl:if test="@rdf:resource">
		  <xsl:variable name="resource" select="@rdf:resource"/>
			...
			<xsl:choose>
			  <xsl:when test="/rdf:RDF/edm:Place[@rdf:about=$resource]/skos:prefLabel/text()">
			    <xsl:element name="{$tgt_property}">
			      <xsl:value-of select="/rdf:RDF/edm:Place[@rdf:about=$resource]/skos:prefLabel/text()"/>
			    </xsl:element>
			  </xsl:when>
			  <xsl:otherwise>
			    <xsl:for-each select="/rdf:RDF/edm:Place[@rdf:about=$resource]/skos:altLabel/text()">
			      <xsl:element name="{$tgt_property}">
			...
			<xsl:if test="/rdf:RDF/edm:Place[@rdf:about=$resource]/skos:note/text()">
				<xsl:element name="{$tgt_property}">
			    <xsl:value-of select="/rdf:RDF/edm:TimeSpan[@rdf:about=$resource]/skos:prefLabel/text()"/>
			  </xsl:element>
			</xsl:if>
