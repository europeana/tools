...
<!--
 This instance of the edm2ese has uses a edmrecord prefix - you have
been warned :) /jaclu
-->
...
<xsl:stylesheet 
	...
	<xsl:template match="/">
		<metadata>		
	  	<xsl:apply-templates select="/xmlsplitted/rdf:RDF/edm:ProvidedCHO"/>		
		</metadata>
	</xsl:template>
	...
	<xsl:template match="rdf:RDF/edm:ProvidedCHO">
		<record>
		  <xsl:variable name="id" select="@rdf:about"/>
			...
			<xsl:for-each select="/edmrecord/rdf:RDF/ore:Aggregation[@rdf:about]">
				<europeana:fake><xsl:value-of select="."/></europeana:fake>
			</xsl:for-each>
			<xsl:for-each select="/edmrecord/rdf:RDF/ore:Aggregation/edm:aggregatedCHO[@rdf:resource=$id]">
	...
	<xsl:template name="map_other_properties">
		...
		<xsl:for-each select="dc:rights">
			<xsl:call-template name="create_elements_from_property">
				<xsl:with-param name="tgt_property">dc:rights</xsl:with-param>
			</xsl:call-template>
		</xsl:for-each>		
	...
	<xsl:template name="create_elements_from_property">
		...
		<xsl:if test="@rdf:resource">
		  <xsl:variable name="resource" select="@rdf:resource"/>
			<xsl:if test="/edmrecord/rdf:RDF/skos:Concept[@rdf:about=$resource]/skos:prefLabel/text()">
			  <xsl:element name="{$tgt_property}">
			    <xsl:value-of select="/edmrecord/rdf:RDF/skos:Concept[@rdf:about=$resource]/skos:prefLabel/text()"/>
			  </xsl:element>
			</xsl:if>
			<xsl:choose>
			  <xsl:when test="/edmrecord/rdf:RDF/edm:Place[@rdf:about=$resource]/skos:prefLabel/text()">
			    <xsl:element name="{$tgt_property}">
			      <xsl:value-of select="/edmrecord/rdf:RDF/edm:Place[@rdf:about=$resource]/skos:prefLabel/text()"/>
			    </xsl:element>
			  </xsl:when>
			  <xsl:otherwise>
			    <xsl:for-each select="/edmrecord/rdf:RDF/edm:Place[@rdf:about=$resource]/skos:altLabel/text()">
			      <xsl:element name="{$tgt_property}">
				<xsl:value-of select="/edmrecord/rdf:RDF/edm:Place[@rdf:about=$resource]/skos:altLabel/text()"/>
			      </xsl:element>
			    </xsl:for-each>
			  </xsl:otherwise>
			</xsl:choose>
			...
			<xsl:if test="/edmrecord/rdf:RDF/edm:Agent[@rdf:about=$resource]/skos:prefLabel/text()">
			  <xsl:element name="{$tgt_property}">
			  </xsl:element>
			</xsl:if>
			<xsl:if test="/edmrecord/rdf:RDF/edm:TimeSpan[@rdf:about=$resource]/skos:prefLabel/text()">
			  <xsl:element name="{$tgt_property}">
			    <xsl:value-of select="/edmrecord/rdf:RDF/edm:TimeSpan[@rdf:about=$resource]/skos:prefLabel/text()"/>
			  </xsl:element>
			</xsl:if>
		  	<xsl:element name="{$tgt_property}">
