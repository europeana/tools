...
import eu.annocultor.converter.ProgressReport;
import eu.annocultor.path.Path;
import eu.annocultor.path.PathMap;
import eu.annocultor.path.PathMap.MatchResult;
import eu.annocultor.path.PathMap.MatchResultIterable;
...
public class ObjectRuleImpl extends ObjectRule
{
	 ...
	 * whole record. These objects need to be processed separately, and the value
	 * of each object property should be treated in the context of this specific
	 * part.
	 * 
	 * A typical example - multiple tags <code>title</code> where each may contain
	 * two subtags: <code>title</code> with the actual text, and
	 ...
	 *          is a Path to a tag that, after preprosessing is completed, should
	 *          hold object id (that implies that you can safely alter it in the
	 *          preprocessor).
	 * 
	 */

	public static ObjectRule makeObjectRule(
			Task task,
			Path objectSeparatingTag,
			Path primaryRecordIdTag,
			Path secondaryRecordIdTag,
			ObjectRule parent,
			boolean registerAsPartListener)
	{
		return new ObjectRuleImpl(task,
				objectSeparatingTag,
				primaryRecordIdTag.isAbsolute() ? primaryRecordIdTag : new Path(objectSeparatingTag, primaryRecordIdTag),
						secondaryRecordIdTag.isAbsolute() ? secondaryRecordIdTag : new Path(objectSeparatingTag, secondaryRecordIdTag),
								parent,
								registerAsPartListener);
	}

	ProgressReport progressReport;

	private ObjectRuleImpl(
			Task task,
			Path recordSeparatingTag,
			Path primaryRecordIdTag,
			Path secondaryRecordIdTag,
			ObjectRule parent,
			boolean registerAsPartListener)
	...
	@Override
	public final boolean selectorPostCondition(DataObject dataObject) throws Exception
	{
		getTask().getReporter().incTotals(dataObject.getDataObjectRule(), Reporter.CATEGORY_CONVERTER, "Total records");
	...
	@Override
	public final boolean selectorPreCondition(DataObject dataObject) throws Exception
	{
		try
		{
	...
	private boolean shouldBeConverted(DataObject dataObject, boolean isPrecondition) throws Exception
	{
		try
		{
			for (DataObjectPreprocessor aSelector : preprocessor)
			{
				if (!(isPrecondition ? aSelector.preCondition(dataObject) : aSelector.postCondition(dataObject)))
					return false;
			}
		}
		catch (Exception e)
		...
		{
			if (parent != null
					&& !(isPrecondition ? parent.selectorPreCondition(dataObject.getParent()) : parent
							.selectorPostCondition(dataObject.getParent())))
				return false;
		}
		catch (Exception e)
		{
			e.printStackTrace();
			throw new Exception("Problem with parent in selector for dataset " + task.getDatasetId());
		}
	...
	public void addRule(PropertyRule rule) throws Exception
	{
		...
		for (PropertyRule realRule : rule.getExpandedRules()) 
		{
			...
			{
				throw new Exception("An instance of rule " + realRule + " has already been used in ObjectRule " + this);
			}
			rulesAlreadyUsed.add(realRule);

			if (realRule.getSourcePath() == null)
			{
				throw new Exception("An instance of rule " + realRule + " has no XML source path assigned");
			}
	...
	@Override
	public final void addAbsRule(Path absolutePath, PropertyRule rule) throws Exception
	{
		if (mapPathToRule.containsPath(absolutePath))
			throw new Exception("Duplicate objectRule for property " + absolutePath + " in the source code.");
		mapPathToRule.put(absolutePath, rule);
		rule.setTask(getTask());
		rule.setSourcePath(absolutePath);
		rule.setObjectRule(this);
		addMapReport(this.task.getDatasetId(), rule);
	}

	@Override
	public final void addRelRule(Path relativePath, PropertyRule rule) throws Exception
	{
		addAbsRule(new Path(recordSeparatingPath, relativePath), rule);
	}

	private final void addMapReport(String dataset, PropertyRule rule) throws Exception
	{
		// keep child rules for reporting
		//addChildRule(rule);

		Stack<PropertyRule> rules = new Stack<PropertyRule>();
		rules.add(rule);

		while (!rules.empty())
		{
			PropertyRule childRule = rules.pop();
			rules.addAll(childRule.getChildRules());

			if ("Ignore".equals(childRule.getAnalyticalRuleClass()))
				continue;
			Class<? extends Rule> class1 = childRule.getClass();
			String className = class1.getName();
			//			Type superClass = class1.getGenericSuperclass();
			//			String superClassName = "";
			//			if (!superClass.toString().contains("java.lang.Class"))
			//			{
			//				if (superClass.getClass().equals(ParameterizedTypeImpl.class))
			//				{
			//					superClassName = ", ext. ";
			//					superClassName += ((ParameterizedTypeImpl) superClass).getRawType().getSimpleName();
			//				}
			//				else
			//				{
			//					String superClassShortName =
			//						superClass.toString().substring(superClass.toString().lastIndexOf('.') + 1);
			//					if (!(superClassShortName.equals("EmptyWriter") || superClassShortName.equals("TripleWriter"))
			//							|| className.contains("$"))
			//					{
			//						superClassName = ", ext. ";
			//						superClassName += superClassShortName;
			//					}
			//				}
			//			}

			getTask().getReporter().inc(rule,
					className.substring(className.lastIndexOf('.') + 1)
					+ ", ext. "
					+ childRule.getAnalyticalRuleClass(),
					"",
					"",
					dataset,
					1);
	...
	private MatchResultIterable<Rule> getRulesForEmptyPath()
	throws Exception {
		return mapPathToRule.answer(new Path(""));
	}

	@Override
	public final void processDataObject(String subject, DataObject dataObject) throws Exception
	{ 
		progressReport.process(subject, dataObject.size(), dataObject.getSeparatingPath().getLastTagExpanded());
		getTask().getReporter().incTotals(dataObject.getDataObjectRule(), Reporter.CATEGORY_CONVERTER, "Passed records");
		...
		for (Path dataPath : uniqueDataPaths)
		{
			for (MatchResult<Rule> mr : getRule(dataPath))
			{
				...
				try
				{
					if (rule.getSourcePath() != null && rule.getSourcePath().isAttributeQuery())
					{
				...
				{
					throw new Exception("Error in ObjectRule in acquiring values for rule " + rule, e);
				}				
				// process each value
				for (XmlValue dataValue : dataValues)
				{
					...
					if (dataValue.getValue().length() > 0)
					{
						try
						{
							...
							ResourceValue subjectOfTriple = new ResourceValue(this.isQualifiedLocalRecordIdentifier() ? task.getTargetNamespace().getUri() : "", subject);
							XmlValue valueOfTriple = dataValue;

							if (dataPath.getPath().equals(Concepts.ANNOCULTOR.PARENT_TO_PART.toString()))
							{
								// special property: set parent as subject
								valueOfTriple = new XmlValue(subjectOfTriple.getValue());
								subjectOfTriple = new ResourceValue(dataValue.getValue());
							}

							nTriple = new Triple(
									subjectOfTriple.getValue(), 
									new Property(dataPath), 
									valueOfTriple, 
									null);
						}
						catch (Exception e)
						{
							throw new Exception("Error in ObjectRule on creating source triple for (original) subject (URL expected) '"
									+ subject + "', value '" + dataValue + "', path " + dataPath, e);
						}
						...
						{
							throw new Exception("Error in ObjectRule on firing rule "
									+ rule + " on source triple " + nTriple, e);
						}
					}
				}
			}
	...
	public final String toString()
	{
		...
		+ " applied to record/tag "
		+ recordSeparatingPath
		+ " with "
		+ mapPathToRule.keySet().size()
		+ " child rules.";
	}
