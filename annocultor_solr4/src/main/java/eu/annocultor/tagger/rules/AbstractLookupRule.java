...
import eu.annocultor.rules.AbstractRenamePropertyRule;
...
public abstract class AbstractLookupRule extends AbstractRenamePropertyRule
{
    ...
    /**
     * For situations when in response to a literal we need to create a term, and then (maybe) link this term to a vocabulary term.
     */
     ...
     *          whether local terms with automatically generated URIs should be created
     *          and linked to external vocabulary terms. or records are linked directly to
     *          external terms.
     */
    public AbstractLookupRule(
            ...
            ProxyTermDefinition proxyTermDefinition)
    {
        ...
        if (properties.contains(mappingCategorySignature))
            throw new RuntimeException("Duplicating lookup property " + mappingCategorySignature);
        ...
        if (dstGraphLiterals == null)
            throw new RuntimeException("null graph is not allowed - use Factory.graphToIgnore() instead");
        ...
        setTermCreator(
                termCreator == null 
                ? new TermCreator(
                        proxyTermDefinition == null ? null : proxyTermDefinition.namespaceLocalTerms.getUri(),	LabelCaseOption.KEEP_ORIGINAL_CASE) 
                : termCreator);
        this.proxyTermDefinition = proxyTermDefinition;
        if (proxyTermDefinition != null && proxyTermDefinition.graphTerms == null)
            throw new RuntimeException("null graph is not allowed - use Factory.graphToIgnore() instead");
    }
    ...
    public void setObjectRule(ObjectRule objectRule)
    {
        ...
        reporter = objectRule.getTask().getReporter();
...
//    	long startfiring = new Date().getTime();
        ...
        if (onPreCondition(triple, dataObject)) {
            ...
            /*
             * Copy the original literal value. Hence, all looked up terms would be
             * a real 'enrichment' and not a replacement. Makes more sense when people
             * would be allowed to see and comment on this enrichment.
             */
            if (getTargetPropertyName() != null)
                super.fire(triple.changeProperty(getTargetPropertyName()), dataObject);

            /*
             * Check completeness of initializaton.
             *  objectRule is not knows at creation time, but known at execution
             */
            if (!objectRuleIsSet)
            {
                throw new Exception("Object rule should be set prior to firing of this rule. Typically it is set in the ObjectRule.addXXX methods.");
            }
            ...
            if (splitPattern == null) {
                ...
                for (String v : triple.getValue().getValue().split(splitPattern)) {
                    labels.add(v);
                }
            }
...
//                    System.out.println("Generating the term URI took " + (new Date().getTime() - uriGeneration) + " ms");
                    ...
                    if (createTermDefinion)
                    {
                        getTermCreator().writeTermDefinition(
                                ...
                                this,
                                new Label(element, proxyTermDefinition.langTermLabel));
                    }
                }
                ...
                if (lastMatch!=null){
                if (!checkIfMissing(lastMatch) && !checkIfAmbigous(lastMatch)) {
                    processLookupMatch(lastMatch, termUri, triple.getSubject(), dataObject, createTermDefinion);				
                }
                reportLookup(lastMatch, element);
                }
            }
//            System.out.println(labels.size() + " Elements took " + (new Date().getTime() - startElements) + " ms");
        }
//        System.out.println("Actual firing took " + (new Date().getTime()-startfiring) + " ms");
    }
    ...
    protected String generateTermUri(String element) throws Exception {
        return Common.generateNameBasedUri(proxyTermDefinition.namespaceLocalTerms, element);
    }
    ...
    protected void reportMissing(String label) throws Exception {
        // not found
        reporter.reportLookupRuleInvocation(
                this, 
                label, 
                null, 
    ...
    protected void reportMatch(TermList terms) throws Exception {
        ...
        reporter.reportLookupRuleInvocation(
                ...
                terms.getFirst().getCode(), 
                VocabularyMatchResult.matched);
    }
    ...
    protected void processLookupMatch(
        ...
        if (proxyTermDefinition == null)
        {
            getTermCreator().writeLinkRecordToTerm(
                    subject,
                    terms.getFirst().getCode(),
                    terms,
                    linkRecordToTerm,
                    graphLinksRecordToTerm,
                    this);
        }
    ...
    /**
     * Find and disambiguates the terms for labels. Specific rules have access to
     * contextual fields that they use to disambiguate.
     * 
     * @param dataObject
     * @param termLabel
     *          term label
     ...
     * @throws Exception
     */
    protected abstract TermList getDisambiguatedTerms(DataObject dataObject, String termLabel, Lang termLang)
    throws Exception;
     ...
     * @throws Exception 
     */
    public static String getFirstValueIfPathExists(DataObject dataObject, Path path) throws Exception {
        if (dataObject == null) {
            return null;
        }
