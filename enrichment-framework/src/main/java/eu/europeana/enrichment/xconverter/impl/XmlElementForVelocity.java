...
public class XmlElementForVelocity {
	...
	public XmlElementForVelocity getFirstChild(String tagName) {
		...
		if (children.length == 0)
			return null;
		return (children[0].getChildren("*").length == 0 && (children[0]
				.getValue() == null || children[0].getValue().isEmpty())) ? null
				: children[0];
	}
	...
	public XmlElementForVelocity[] getChildren(String tagName) {
		...
		for (int i = 0; i < children.getLength(); i++) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE
					&& (tagName.equals("*") || tagName.equals(node
							.getNodeName()))) {
	...
	public String getValue() {
		if (element.getFirstChild() != null
				&& (element.getFirstChild().getNodeType() == Node.TEXT_NODE || element
	...
	public String getAttribute(String attName) throws Exception {
		NST nst = new NST(attName);
		return element.getAttributeNS(nst.nsURI, nst.localName).isEmpty() ? null
				: element.getAttribute(attName);
	}

	private class NST {

		public NST(String tag) throws Exception {
			this.nsURI = "";
			this.localName = tag;
	 ...
	 * @throws Exception
	 */
	public static String enquote(String value) throws Exception {
		if (value == null)
			return null;
