...
import eu.europeana.enrichment.xconverter.api.GeneratedConverterInt;
...
public class Converter {
	...
	public static void main(String... args) throws Exception {
		if (args.length != 2) {
			System.err
					.println("Error in parameters, expected <profile> <work dir>");
			System.exit(1);
		}
		int result = run(new File(args[0]), new File(args[1]), new PrintWriter(
				System.out));
		System.exit(result);
	}
	...
	static File compile(File profileFile, File workDir, PrintWriter console,
			OutputStream javaOutputStream) throws Exception {
		if (!profileFile.exists()) {
			throw new IOException("Cannot find xconverter: "
					+ profileFile.getCanonicalPath());
		}

		// everything happens in the working dir named according to the profile
		// file name
		...
		VelocityContext context = converter.run(profileFile.getParentFile(),
				new FileInputStream(profileFile),
				"xconverter/XConverterGenerator.vm", javaOutputStream, work);
		...
		File javaFile = new File(profile,
				XConverter2Java.GENERATED_CONVERTER_CLASS_NAME + ".java");
		if (javaFile.exists()) {
			javaFile.delete();
		}
		...
		// compile it
		String cpJars = System.getProperty("java.class.path")
				+ File.pathSeparatorChar
				+ System.getProperty("sun.boot.class.path")
				+ File.pathSeparatorChar
				+ System.getenv("ANNOCULTOR_TOOLS_JAR")
				+ File.pathSeparatorChar + "mavenbin/";
		...
		console.println("Running generated converter");// , classpath: " +
														// cpJars);
		console.flush();
		...
		try {
			...
			if (compiler == null) {
				throw new Exception(
						"Cannot find Java compiler. Are you running Java 6 JDK? Detected "
			...
			Iterable<? extends JavaFileObject> cu = fileManager
					.getJavaFileObjectsFromFiles(Arrays.asList(javaFile));
			boolean success = compiler.getTask(console, fileManager, null,
					Arrays.asList("-cp", cpJars), null, cu).call();
	...
	static int run(File profileFile, File workDir, PrintWriter console,
			OutputStream javaOutputStream) throws Exception {
		try {
			File profile = compile(profileFile, workDir, console,
					javaOutputStream);
			if (profile != null) {
				// run it
				...
				ClassLoader cl = new URLClassLoader(new URL[] { profile.toURI()
						.toURL() });

				// Load in the class
				Class cls = cl
						.loadClass(XConverter2Java.GENERATED_CONVERTER_CLASS_NAME);

				// Create a new instance of the new class
				GeneratedConverterInt instance = (GeneratedConverterInt) cls
						.newInstance();
				if (instance == null) {
