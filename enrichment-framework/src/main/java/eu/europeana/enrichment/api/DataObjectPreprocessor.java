...
public abstract class DataObjectPreprocessor
{
	 ...
	 * <code>dataObject</code> such as generated object id's or other fields that
	 * are easier to compute here and then use by property conversion rules
	 * (override it!).
	 * 
	 * @param dataObject
	 * @return
	 ...
	 * and {@linkplain #process(DataObject)} but before property rules are applied
	 * to this data object; it is typically used to filter some records out based
	 * on the values of the <code>dataObject</code> that were changed by the
	 * {@linkplain #process(DataObject)} method (override it!).
	 * 
