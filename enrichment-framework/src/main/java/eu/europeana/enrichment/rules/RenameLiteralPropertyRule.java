...
public class RenameLiteralPropertyRule extends AbstractRenamePropertyRule {
	...
	/**
	 * Enforces a fixed <code>xml:lang</code>. Use it if you know the language
	 * of this property, when it is not specified in the source XML, or may be
	 * specified incorrectly.
	 * 
	 * @param dstProperty
	 *            destination property
	 * @param dstLangCode
	 *            2-letter code of the xml:lang to set in the destination
	 *            property, e.g. "nl"; empty value keeps the original language
	 * @param dstGraph
	 *            destination graph
	 */
	...
	public RenameLiteralPropertyRule(
			...
			Property dstProperty, String dstLangCode, Graph dstGraph) {
	...
	public RenameLiteralPropertyRule(Property propertyName,
			String enforcedLang, Graph graph) {
	...
	public RenameLiteralPropertyRule(
			@AnnoCultor.XConverter.sourceXMLPath Path srcPath,
			Property dstProperty, Path srcPathToLang, String defaultLang,
			Graph dstGraph) {
	...
	public void fire(Triple triple, DataObject dataObject) throws Exception {
		...
		if (t.getValue() instanceof LiteralValue) {
			...
			if (enforcedLang == null) {
				// dynamic lang
				String dynamicLang = pathToLang == null ? null
						: convertLanguageToIsoCode(getLangValue(dataObject));
				if (value.getLang() == null) {
					if (dynamicLang != null) {
						// use dynamic
						t = t.changeValue(new LiteralValue(value.getValue(),
								dynamicLang));
			...
			throw new Exception(this.getClass().getName()
					+ " is applied to a non-literal value");
		}
	...
	protected String convertLanguageToIsoCode(String language) throws Exception {
		return language;
	}
