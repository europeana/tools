...
public class ProgressReport {
	...
	public ProgressReport() throws Exception {
		...
		timer.start();
		nextStop = 100;
		initalMemoryConsumption = 0;// Runtime.getRuntime().totalMemory() -
	...
	private long assignNextStop() {
		...
		for (long threshold : thresholds) {
			if (nextStop >= threshold)
				return threshold;
		}
		return 250;
	}

	public void process(String currentId, long incrementStep,
			String signatureOfThingInProgress) {
		passedDataObjects++;
		...
		if (passedDataObjects >= nextStop) {
			...
			long memoryComsumption = Runtime.getRuntime().totalMemory()
					- Runtime.getRuntime().freeMemory()
					- initalMemoryConsumption;
			log.info(String
					.format("Progress of %s: %d records in %d sec, using %d Mb RAM (%d B/record), sliding av. record length %d lines, now at id: %s ",
