...
from update_req_stats import UpdateRequestStats
from syncmanager import SyncManager, SyncLeftovers
...
class RequestList(object):
    ...
    def has_content(self):
        return bool(self.req_list)
    
    def repopulate_requests(self):
        cursor = connection.cursor()
        cursor.execute('SELECT id from dummy_ingester_request order by id')
        self.req_list = []
        for row in cursor.fetchall():
            self.req_list.append(row[0])
        return
...
class UriCreate(sip_task.SipTask):
    ...
    def run_it(self):
        ...
        for mdr_id, status, source_data, request_id in cursor.fetchall():
            record_count += 1
            if status not in (base_item.MDRS_CREATED,
                                  base_item.MDRS_IDLE,
...
class UriValidateSave(UriHandlerBase):
    ...
    def run_it(self):
        ...
        for row in cursor.fetchall():
            ...
            try:
                self.task_time_to_show(idx, terminate_on_high_load=True)
                #if 'RequestParseNew' in sip_task.RUNNING_PLUGINS:
                #    break # dont run this when RequestParseNew is running
            except sip_task.SipSystemOverLoaded:
                 # Terminate in a controled fashion so we can do cleanup
                break
