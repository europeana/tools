...
public class RubbishTermDetector {
	...
	private static abstract class PotentialProblemPredicate implements
			Predicate {
			...
			if (!warned && evaluate(term)) {
				...
				if (occurrences.size() > threshold) {
					log.warn("Potential problem '"
							+ name
							+ "' with terms that may cause OutOfMemory exception and more. Happened on \n"
							+ Utils.show(occurrences, "\n"));
					log.warn("This problem has happened aready with "
							+ threshold
							+ " different terms and may happen with others, but no more warnings will be made: YOU ARE WARNED");
					warned = true;
				}
			}
		}
	...
	private static class BracketProblemPredicate extends
			PotentialProblemPredicate {
		...
		public boolean evaluate(Object arg) {
			return arg.toString().contains(opening) != arg.toString().contains(
					closing);
		}

	};

	protected PotentialProblemPredicate isLongTermProblem = new PotentialProblemPredicate(
			"Long term (80)") {
	...
	protected PotentialProblemPredicate isManyWordsProblem = new PotentialProblemPredicate(
			"Too many words (7)") {
	...
	protected PotentialProblemPredicate isSemicolonProblem = new PotentialProblemPredicate(
			"Contains ';'") {
	...
	public void warnOnStrangeTerms(String term) {
		// here we need 'give all problems' function that is missing in jakarta
		// commons
