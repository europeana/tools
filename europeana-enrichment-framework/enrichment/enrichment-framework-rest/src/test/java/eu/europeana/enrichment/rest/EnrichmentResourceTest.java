...
public class EnrichmentResourceTest extends JerseyTest {
	...
	public void testEnrich() {
		try {
			...
			when(enricher.tagExternal(Mockito.anyList())).thenReturn(
					wrapResp);
			
			Form form = new Form();
			form.param("input", new ObjectMapper().writeValueAsString(vals));
			form.param("toXml", Boolean.toString(false));
			Response response = target("/enrich").request(MediaType.APPLICATION_JSON).post(
					Entity.entity(form, MediaType.APPLICATION_FORM_URLENCODED),
					Response.class);

			EntityWrapperList wrapList = new ObjectMapper().readValue(response.readEntity(String.class),
					EntityWrapperList.class);
